...
const { semver, getPluginLink, toShortPluginId, loadModule } = require('@vicli/cli-shared-utils')
...
class GeneratorAPI {
  ...
  get cliServiceVersion () {
    ...
    const servicePkg = loadModule(
      '@vicli/cli-service/package.json',
  ...
  assertCliServiceVersion (range) {
    ...
    throw new Error(
      `Require @vicli/cli-service "${range}", but was loaded with "${this.cliServiceVersion}".`
    )
  }
